...
public class ChainMakerConnectionFactory {
    ...
    public static ChainMakerConnection build(String stubConfigPath,
                                             String configName) throws Exception {
        ScheduledExecutorService executorService = new ScheduledThreadPoolExecutor(
                4,
                new CustomizableThreadFactory("tmpChainMakerConn-")
        );
    ...
    public static ChainMakerConnection build(ChainMakerStubConfig chainMakerStubConfig,
                                             AbstractClientWrapper clientWrapper) {
    ...
    public static ChainMakerConnection build(ChainMakerStubConfig chainMakerStubConfig,
        ...
        connection.addProperty(
                ChainMakerConstant.CHAIN_MAKER_PROPERTY_CHAIN_ID, chainMakerStubConfig.getChain().getChainId());
        connection.addProperty(
                ChainMakerConstant.CHAIN_MAKER_PROPERTY_AUTH_TYPE, chainMakerStubConfig.getChain().getAuthType());
        connection.addProperty(
                ChainMakerConstant.CHAIN_MAKER_PROPERTY_STUB_TYPE, chainMakerStubConfig.getCommon().getType());
